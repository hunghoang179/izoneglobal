                                    ...
                                    <li class="<?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item product-info', $cart_item, $cart_item_key)); ?>">
                                        <div class="p-left">
                                            ...
                                            echo apply_filters('woocommerce_cart_item_remove_link', sprintf(
                                                            '<a href="%s" class="remove remove_link" title="%s" data-product_id="%s" data-product_sku="%s"></a>', esc_url(WC()->cart->get_remove_url($cart_item_key)), __('Remove this item', 'kutetheme'), esc_attr($product_id), esc_attr($bag_product->get_sku())
                                                    ), $cart_item_key);
                                            ?>
...
if (!function_exists('kt_show_product_meta')) {
    ...
    function kt_show_product_meta() {
        ...
        if ($product->is_in_stock())
            $availability = esc_attr__('In stock', 'kutetheme');
        if (!$product->is_in_stock())
            $availability = esc_attr__('Out of stock', 'kutetheme');
        ?>
    ...
    function kt_get_hot_product_tags($number = 3) {
        ...
        <?php if ($terms): ?>
            ...
            <?php foreach ($terms as $term): ?>
                ...
                $i++;
                ?>
                        <a href="<?php echo esc_url(get_term_link($term)); ?>"><?php echo esc_html($term->name); ?><?php if ($i < $number) esc_html_e(', ', 'kutetheme');
                else esc_html_e(' ..', 'kutetheme'); ?></a>
            <?php endforeach; ?>
                </p>
            </div>
        <?php endif; ?>
    ...
    if ($args->theme_location == 'topbar_menuright') {
        ...
        <?php if (kt_is_wc()): ?>
            ...
            <?php else: ?>
                ...
                <li><a href="<?php echo get_site_url();?>/wp-login.php?loginFacebook=1&redirect=<?php echo get_site_url();?>" onclick="window.location = '<?php echo get_site_url();?>/wp-login.php?loginFacebook=1&redirect='+window.location.href; return false;"><img src="<?php echo get_template_directory_uri ();?>/images/facebook-login-blue.png" title="login with facebook" /></a></li>
            <?php endif; ?>
        <?php endif; ?>
        <?php
    }
